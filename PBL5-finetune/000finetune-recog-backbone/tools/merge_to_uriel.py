...
vào thư mục models/merged/uriel/ để chuẩn bị đóng gói.
...
DEFAULT_ONNX = FINETUNE_ROOT / "models" / "finetuned" / "w600k_r50.onnx"
DEFAULT_WITHOUT_BACKBONE_DIR = FINETUNE_ROOT / "models" / "without_backbone"
DEFAULT_URIEL_DIR = FINETUNE_ROOT / "models" / "merged" / "uriel"
...
def main() -> None:
    ...
    # Nếu có file .onnx.data thì copy kèm sang với đúng tên
    onnx_data_src = args.onnx.with_suffix(args.onnx.suffix + '.data')
    if onnx_data_src.exists():
        onnx_data_dst = dest.with_suffix(dest.suffix + '.data')
        shutil.copy2(onnx_data_src, onnx_data_dst)
        print(f"  Copied: {onnx_data_src.name} → {onnx_data_dst.name}")
    else:
        print("  Không có file .onnx.data kèm theo.")
