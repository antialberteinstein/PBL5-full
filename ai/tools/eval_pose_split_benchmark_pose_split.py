"""Evaluate pose-split classify latency using pose-specific collections."""
...
LAB_NAME = "Pose Split Benchmark - Pose Split Collections"
LAB_FILE_NAME = "pose_split_benchmark_pose_split"
DB_NAME = "pose_benchmark_pose_split.db"
...
def extract_embeddings(paths: List[Path]) -> Tuple[List[np.ndarray], List[str], Counter, List[float], int, int]:
    ...
    for idx, img_path in enumerate(paths, start=1):
        ...
        pose_labels.append(predicted)
        pose_counts[predicted] += 1
        latencies_ms.append((end - start) * 1000.0)

    return embeddings, pose_labels, pose_counts, latencies_ms, skipped, no_face
...
def main() -> None:
    ...
    embeddings, pose_labels, pose_counts, recog_lat_ms, skipped, no_face = extract_embeddings(paths)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        classifier = CosineClassifier(database_path=db_path)
        pipeline = ClassificationPipeline(pca, scaler, classifier)

        embeddings_by_pose: dict[str, List[np.ndarray]] = {p: [] for p in POSES}
        for emb, pose_name in zip(embeddings, pose_labels):
            embeddings_by_pose[pose_name].append(emb)

        for pose_name, emb_list in embeddings_by_pose.items():
            if not emb_list:
                continue
            pipeline.fit_with_pose(SINGLE_LABEL, emb_list, pose_name)

        for emb, pose_name, recog_ms in zip(embeddings, pose_labels, recog_lat_ms):
            start = perf_counter()
            pipeline.predict_use_pose_with_score(emb, pose_name)
            end = perf_counter()
