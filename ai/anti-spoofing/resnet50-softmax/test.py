...
def main() -> None:
    ...
    parser.add_argument("--config_path", default="resnet50-softmax/test.yml", help="Path to config file")
    ...
    ckpt_path = Path(config.get("ckpt_path", "models/anti_spoofing/resnet50_softmax/resnet50_softmax_best.pt"))
    ...
    with torch.no_grad():
        while True:
            ...
            for face in faces:
                ...
                # Lấy bounding box và thêm padding
                x1, y1, x2, y2 = face.bbox.astype(int)
                w, h = x2 - x1, y2 - y1
                pad_w = int(w * FACE_PAD_RATIO)
                pad_h = int(h * FACE_PAD_RATIO)
                x1_pad = max(x1 - pad_w, 0)
                y1_pad = max(y1 - pad_h, 0)
                x2_pad = min(x2 + pad_w, frame.shape[1])
                y2_pad = min(y2 + pad_h, frame.shape[0])
                face_crop = frame[y1_pad:y2_pad, x1_pad:x2_pad]

                # Căn chỉnh khuôn mặt bằng 5 điểm landmark trên ảnh đã padding
                # Cần điều chỉnh lại landmark tương ứng vùng crop
                kps = face.kps.copy()
                kps[:, 0] -= x1_pad
                kps[:, 1] -= y1_pad
                aligned_crop = face_align.norm_crop(face_crop, landmark=kps, image_size=FACE_CROP_SIZE[0])
